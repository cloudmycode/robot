...
I2cDevice::I2cDevice(i2c_master_bus_handle_t i2c_bus, uint8_t addr) {
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "添加I2C设备失败，地址: 0x%02X, 错误: %s", addr,
                 esp_err_to_name(ret));
        i2c_device_ = nullptr;
    } else {
...
uint8_t I2cDevice::ReadReg(uint8_t reg) {
    uint8_t buffer[1];
    esp_err_t ret = i2c_master_transmit_receive(i2c_device_, &reg, 1, buffer, 1,
                                                I2C_TIMEOUT_MS);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "I2C读寄存器失败，寄存器: 0x%02X, 错误: %s", reg,
                 esp_err_to_name(ret));
        return 0;
    }
    return buffer[0];
...
void I2cDevice::ReadRegs(uint8_t reg, uint8_t* buffer, size_t length) {
    ...
    esp_err_t ret = i2c_master_transmit_receive(i2c_device_, &reg, 1, buffer,
...
void I2cDevice::WriteRegs(uint8_t reg, const uint8_t* buffer, size_t length) {
    ...
    // 准备发送缓冲区：寄存器地址 + 数据
    std::vector<uint8_t> send_buffer(length + 1);
    send_buffer[0] = reg;
    std::copy(buffer, buffer + length, send_buffer.begin() + 1);

    esp_err_t ret = i2c_master_transmit(i2c_device_, send_buffer.data(),
                                        send_buffer.size(), I2C_TIMEOUT_MS);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG,
                 "I2C写多个寄存器失败，起始寄存器: 0x%02X, 长度: %d, 错误: %s",
                 reg, length, esp_err_to_name(ret));
    }
}