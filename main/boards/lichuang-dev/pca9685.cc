    ...
    : I2cDevice(i2c_bus, addr), pwm_frequency_(DEFAULT_PWM_FREQ)
{
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "PCA9685设备未找到，地址: 0x%02X, 错误: %s", addr, esp_err_to_name(ret));
        return;
    }
...
void Pca9685::Initialize()
{
    ...
    ESP_LOGI(TAG, "PCA9685初始化完成，PWM频率: %.1f Hz, 预分频值: %d", pwm_frequency_, prescale);
}
...
void Pca9685::SetServoAngle(uint8_t channel, int angle)
{
    ...
    {
        ESP_LOGW(TAG, "角度超出范围 (%d-%d): %d", SERVO_MIN_ANGLE, SERVO_MAX_ANGLE, angle);
        angle = (angle < SERVO_MIN_ANGLE) ? SERVO_MIN_ANGLE : SERVO_MAX_ANGLE;
    }

    // 将角度转换为脉冲宽度
    uint16_t pulse_us = SERVO_MIN_PULSE +
                        ((angle - SERVO_MIN_ANGLE) * (SERVO_MAX_PULSE - SERVO_MIN_PULSE)) /
                            (SERVO_MAX_ANGLE - SERVO_MIN_ANGLE);

    SetServoPulse(channel, pulse_us);
    ESP_LOGI(TAG, "通道 %d 舵机角度设置为 %d° (脉冲宽度: %d us)", channel, angle, pulse_us);
}
...
void Pca9685::SetPWM(uint8_t channel, uint16_t on, uint16_t off)
{
    ...
    while (retry_count < max_retries)
    {
        ...
        {
            ESP_LOGD(TAG, "通道%d PWM设置成功: ON=0x%04X, OFF=0x%04X", channel, on, off);
            return;
        }
        else
        {
            ESP_LOGW(TAG, "通道%d PWM设置验证失败，重试 %d/%d", channel, retry_count + 1, max_retries);
            retry_count++;
            vTaskDelay(pdMS_TO_TICKS(10));
        }
    }
...
void Pca9685::SetServoPulse(uint8_t channel, uint16_t pulse_us)
{
    ...
    {
        ESP_LOGW(TAG, "脉冲宽度超出范围 (%d-%d us): %d", SERVO_MIN_PULSE, SERVO_MAX_PULSE, pulse_us);
        pulse_us = (pulse_us < SERVO_MIN_PULSE) ? SERVO_MIN_PULSE : SERVO_MAX_PULSE;
    }

    float pwm_period_us = 1000000.0f / pwm_frequency_;
    uint16_t pwm_units = static_cast<uint16_t>((pulse_us * 4096.0f) / pwm_period_us);

    ESP_LOGD(TAG, "通道%d: 脉冲宽度=%dus, PWM周期=%fus, PWM单位=%d",
             channel, pulse_us, pwm_period_us, pwm_units);
